...
class Trainer:
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(self.train_loader)):
            ...
            # self._assert_finite(outputs, "model outputs")
            loss = self.criterion(outputs, target)  # Compute loss
            # self._assert_finite(loss.detach(), "loss")
            loss.backward()  # Backward pass

             # Check gradients BEFORE any clipping/step
            # for name, p in self.model.named_parameters():
            #     if p.grad is not None:
            #         self._assert_finite(p.grad, f"gradients of {name}")
            ...
            # for name, p in self.model.named_parameters():
            #     self._assert_finite(p.data, f"parameters of {name}")
    ...
    def test(self):
        ...
        with torch.no_grad():  # Disable gradient calculation
            for data, target, sub_ids in self.test_loader:
                # print(data.shape)
                # print(target)
                # print(sub_ids)
                # input()
    ...
    def train(self, epochs):
        ...
        for epoch in tqdm(range(0,epochs), ncols = 120):
            ...
            self.train_val_metrics.append(f"Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}, Val Sen: {val_sen:.4f}, Val Spe: {val_spe:.4f}, Val MCC: {val_mcc:.4f}, Val F1_neg: {val_f1neg:.4f}, Val F1_pos: {val_f1pos:.4f}")
